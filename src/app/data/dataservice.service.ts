...
export class DataserviceService {
    ...
    deletePhotos = (key, url) => {
        ...
        this.deletestatus = this.afStorage.storage.refFromURL(url).delete().then(() => true).catch(error => error);
    ...
    uploadPhoto(event: FileList, photobio): void {
        ...
        if (this.file.type.split('/')[0] !== 'image') {
            this.toastr.warning('UnSuppported File', 'Please Select Image File', {
                timeOut: 1000
            });
        } else {
